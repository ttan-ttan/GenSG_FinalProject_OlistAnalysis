...
def test_casts_types_and_parses_timestamps(spark):
    ...
    row = clean_order_reviews(df).collect()[0]  # collect() pulls the data back to Python so we can inspect it
    ...
    assert row["review_creation_date"] == datetime(2018, 1, 18, 0, 0, 0)
    assert row["review_answer_timestamp"] == datetime(2018, 1, 18, 21, 46, 59)


def test_blank_strings_become_null_and_flags_are_derived(spark):
    df = spark.createDataFrame(
        [_row(review_id="r2", review_comment_title="", review_comment_message="Great product!")],
        schema=RAW_SCHEMA,
    )
...
def test_drops_rows_with_invalid_score(spark):
    df = spark.createDataFrame(
        [_row(review_id="r_bad", review_score="9"), _row(review_id="r_good", review_score="4")],
        schema=RAW_SCHEMA,
    )
...
def test_deduplicates_on_review_id_keeping_latest_answer(spark):
    ...
    df = spark.createDataFrame(
        [
            _row(review_id="r1", order_id="o1", review_answer_timestamp="2018-01-18 10:00:00"),
            _row(review_id="r1", order_id="o2", review_answer_timestamp="2018-01-20 10:00:00"),
        ],
        schema=RAW_SCHEMA,
    )
    rows = clean_order_reviews(df).collect()
    assert len(rows) == 1                # only one row should remain for r1
    assert rows[0]["order_id"] == "o2"   # it should be the one with the LATER answer time
