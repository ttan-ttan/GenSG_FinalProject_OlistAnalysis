...
def _row_rules() -> dict:
    ...
    rules.update({
        "product_id not 32-char lowercase hex": ~col("product_id").rlike(PRODUCT_ID_PATTERN),

        # "unknown" english allowed only for missing or known-untranslated categories
        "new untranslated category": (col("product_category_name_english") == UNKNOWN_CATEGORY)
        ...
        "is_dims_missing inconsistent": col("is_dims_missing") != (
            col("product_weight_g").isNull() | col("product_length_cm").isNull()
            | col("product_height_cm").isNull() | col("product_width_cm").isNull()),
        ...
        "product_volume_cm3 mismatch": col("product_volume_cm3") != (
            col("product_length_cm").cast("long") * col("product_height_cm").cast("long")
        ...
        "product_description_length not positive": col("product_description_length") <= 0,
    })
    return rules
